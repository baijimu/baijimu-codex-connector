...
Windows initialization verifies the isolated profile through the documented Codex app-server JSON-RPC sequence. It waits for the initialize response, API-key login response, `account/login/completed`, and `account/updated` before issuing the final `account/read`; it does not infer success from output text or a fixed sleep. It then persists and reads back the desktop locale (default `zh-CN`), checks `windowsSandbox/readiness`, and, when required, starts the official elevated sandbox setup and waits for `windowsSandbox/setupCompleted` before initialization can succeed. The user must approve the Windows UAC prompt. Protocol errors, early process exit, timeout, a rejected UAC request, incomplete sandbox setup, and a non-`apiKey` final account are reported separately with credentials masked.
