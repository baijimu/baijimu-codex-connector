...
It is installed and supervised by `bridge-agent`. Bridge Agent finishes Connector installation without waiting for Codex initialization. On first use, the workspace-management page automatically starts initialization for the currently authorized workspace and shows the official installer's step and download progress. On macOS and Windows, setup deliberately defers the official desktop launch until the workspace-scoped credential and active `CODEX_HOME` selection are complete, then opens the installed ChatGPT/Codex application as the current interactive user and verifies the real desktop process. Desktop launch never creates or opens a synthetic project and does not use window visibility as a readiness signal. A failed initialization stops without looping and exposes an explicit repair action in the same page. Credential issuance, exact workspace validation, the official Codex installer, configuration, smoke tests, and process/state-directory verification run inside this application. Bridge Agent never receives the LLM key.
